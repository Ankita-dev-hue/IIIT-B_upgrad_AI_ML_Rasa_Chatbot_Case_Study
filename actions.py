...
class ActionCheckLocation(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		city_list = ['Ahmedabad','Bangalore','Chennai','Delhi','Hyderabad','Kolkata','Mumbai','Pune','Agra','Ajmer',
		...
		if location_results == 0 or loc.lower() not in [x.lower() for x in city_list]:
			#dispatcher.utter_message('We do not operate in that area yet. Can you specify some other location?')
			return [SlotSet('check_op',"not_found")]
		else:
			return [SlotSet('check_op',"found")]
	...
	def run(self, dispatcher, tracker, domain):
		...
		if len(sorted_dict) == 0:
			...
			for k, v in sorted_dict.items():
				...
				if (i == 10):
					break;
		sender_email = 'recommendationsrestaurant@gmail.com'
